...
function 战斗处理类:取基础固伤伤害(编号,目标,名称,等级,系数)
          --系数= {初始系数=1,叠加系数=0,初始伤害=0,防御系数=1,忽视防御=0, ---初始计算
          --       暴击系数=1,暴击增加=0,暴伤系数=1,暴伤增加=0,            ---暴击计算
          --       结果系数=1,结果伤害=0,特效={}}                                  ---最终计算
          if 名称=="龙吟" then
            self:减少魔法(目标,math.floor((等级+10)*0.75))
            return math.floor(等级+10)--气血伤害
          end
          if self.参战单位[编号].奇经八脉.灵变 and  self.参战单位[目标].类型=="角色" then
              系数.初始伤害 = 系数.初始伤害 + 50
          end
          if self.参战单位[编号].奇经八脉.借灵  then
              系数.初始伤害 = 系数.初始伤害 + 玩家数据[self.参战单位[编号].玩家id].经脉:武器伤害处理(24)
          end
          if 名称~="龙吟" then
              系数.初始伤害 = 系数.初始伤害 + self.参战单位[编号].武器伤害 * 0.1 + self.参战单位[编号].速度*0.2 +  self.参战单位[编号].法伤*0.05 + self.参战单位[编号].伤害*0.05
          end
          if self.参战单位[目标].奇经八脉.回旋 then
              系数.初始伤害 = 系数.初始伤害 - 65
          end

          if self.参战单位[编号].灵宝乾坤水卷 then
             系数.结果系数 = 系数.结果系数 +  self.参战单位[编号].灵宝乾坤水卷
          end
          if self.参战单位[编号].奇经八脉.冥煞 and 取随机数()<=5 then
            系数.结果系数 = 系数.结果系数 + 1
          end
          if self.参战单位[编号].神话词条 and self.参战单位[编号].神话词条.鸿蒙元魂 then
              系数.结果系数 = 系数.结果系数 + self.参战单位[编号].神话词条.鸿蒙元魂 * 0.04
          end
          if self.参战单位[编号].战斗赐福 and self.参战单位[编号].战斗赐福.固伤结果>0 then
             系数.结果系数 = 系数.结果系数 + self.参战单位[编号].战斗赐福.固伤结果 * 0.01
          end
          if self.参战单位[编号].奇经八脉.聚魂 and self.参战单位[编号].门派=="无底洞" then
              系数.结果系数 = 系数.结果系数 + 0.08
          end
          if self.参战单位[编号].类型=="角色" and (self.参战单位[编号].门派 == "阴曹地府" or self.参战单位[编号].门派 == "女儿村"
              or self.参战单位[编号].门派 == "盘丝洞" or self.参战单位[编号].门派 == "普陀山" or self.参战单位[编号].门派 == "无底洞") then
                系数.结果系数 = 系数.结果系数 + 玩家数据[self.参战单位[编号].玩家id].角色:取生活技能等级("暗器技巧")/1000
          end

          if self.参战单位[编号].奇经特效.金莲  then
             系数.结果系数 = 系数.结果系数 - 0.3
          end
          if self.参战单位[目标].神话词条 and self.参战单位[目标].神话词条.超然物外 then
              系数.结果系数 = 系数.结果系数 - self.参战单位[目标].神话词条.超然物外 * 0.04
          end
          if self.参战单位[目标].战斗赐福 and self.参战单位[目标].战斗赐福.固伤减免>0 then
             系数.结果系数 = 系数.结果系数 - self.参战单位[目标].战斗赐福.固伤减免 * 0.01
          end

          -----------------计算介绍
          return math.floor((等级 * 3.5 +self.参战单位[编号].固定伤害)*系数.初始系数 + 系数.初始伤害)
end



function 战斗处理类:固伤技能计算(编号,名称,等级,额外命令,伤害比,境界)
        local 前置 = {
            结尾 = 0,
            重复攻击 = false,
            目标数 = 1,
        ...
        for n=1,前置.目标数 do
            ...
            if 前置.重复攻击 and n ~= 1 then
                ...
                self:固伤循环处理(编号,前置.目标组[n],n,名称,前置.目标数,等级,前置.流程,1,伤害比,境界)
            else
                self:固伤循环处理(编号,前置.目标组[n],n,名称,前置.目标数,等级,前置.流程,n,伤害比,境界)
            end

            添加流程(self:固伤循环中处理(编号, 前置.目标组[n]))
...
function 战斗处理类:固伤循环处理(编号,目标,次数,名称,总数,等级,流程,挨打,伤害比,境界)
          self.伤害输出 = 0
          ...
          if 次数==1 then
             ...
             if self.参战单位[编号].法术状态.莲心剑意 then
                基础.结果伤害 = 基础.结果伤害 + self.参战单位[编号].法术修炼*3
             end
          end
          ...
          基础.特效 = {}
          local 计算 = self:取基础固伤伤害(编号,目标,名称,等级,基础)
          if 总数<=5 then
              计算 = 计算 * (1 - (总数-1) * 0.04)
          else
              计算 = 计算 * (0.84 - (总数-5) * 0.02)
          end
          if 伤害比 then 计算 = 计算*伤害比 end
          计算 = math.floor(计算)
          local 结果 = self:取计算结果(编号,目标,计算,nil,流程,挨打,基础,名称,等级,"固伤")
...
function 战斗处理类:固伤循环中处理(编号,目标)  ---------和法伤大部分相同
          local 添加流程={}
