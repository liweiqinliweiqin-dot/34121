...
function 战斗处理类:道具存在(编号,道具)

   local 道具数据= 玩家数据[self.参战单位[编号].玩家id].角色.数据.道具

   for i=1,100 do
         if 道具数据[i]~=nil and 玩家数据[self.参战单位[编号].玩家id].道具.数据[道具数据[i]]~=nil  then
            if 玩家数据[self.参战单位[编号].玩家id].道具.数据[道具数据[i]].名称==道具 then
             return  true
            end
         end
  ...
  self.奇袭编号={}
   ...
   for k,v in pairs(self.参战玩家) do
      ...
      if self.参战单位[编号].类型=="角色"  then
               if self.参战单位[编号].命魂之玉奇袭特技 and  self.参战单位[编号].命魂之玉奇袭特技[1] then
                   for i=1,#self.参战单位[编号].命魂之玉奇袭特技 do
                       if self:特技存在(编号,self.参战单位[编号].命魂之玉奇袭特技[i].参数) then
                          table.insert(data.特技,self.参战单位[编号].命魂之玉奇袭特技[i].参数)
                          通过=true
                       end
                   end
                ...
                if self.参战单位[编号].命魂之玉奇袭道具 and  self.参战单位[编号].命魂之玉奇袭道具[1] then
                 for i=1,#self.参战单位[编号].命魂之玉奇袭道具 do
                     if self:道具存在(编号,self.参战单位[编号].命魂之玉奇袭道具[i].参数) then
                        table.insert(data.道具,self.参战单位[编号].命魂之玉奇袭道具[i].参数)
                        通过=true
                     end
                 end
                ...
                if 通过 then
                  发送数据(v.连接id,5800,{数据=data,id=编号})
                end

                if self.参战单位[编号].奇袭法术  then
                   print(self.参战单位[编号].奇袭法术)
                    if 取随机数(1,1000)<=self.参战单位[编号].奇袭法术*1000 then
                      if self.参战单位[编号].队伍==self.队伍区分[1] then
                        table.insert(self.奇袭队伍[1],编号)
                      end
                       table.insert(self.奇袭队伍[2],编号)
                    end
                end
      end
  end

    if self.奇袭队伍[1][1] then
        local 编号=self.奇袭队伍[1][取随机数(1,#self.奇袭队伍[1])]
        local 技能=self.参战单位[编号].主动技能

        发送数据(玩家数据[self.参战单位[编号].玩家id].连接id,5801,{数据=技能,id=编号})


    end
    if self.奇袭队伍[2][1] then
        local 编号=self.奇袭队伍[2][取随机数(1,#self.奇袭队伍[2])]
        local 技能=self.参战单位[编号].主动技能
        table.insert(self.奇袭编号,编号)
        发送数据(玩家数据[self.参战单位[编号].玩家id].连接id,5801,{数据=技能,id=编号})
   end
...
function 战斗处理类:超级特技道具计算(编号)
      ...
      local 添加流程
        local 目标=self.参战单位[编号].超级特技指令.目标
        local 道具=self.参战单位[编号].超级特技指令.参数
        ...
        local 使用类型
        local 类型映射 = {
            加血 = {"金创药","小还丹","千年保心丹","金香玉","五龙丹"},
            加魔 = {"翡翠豆腐","佛跳墙","蛇蝎美人","风水混元丹","定神香","十香返生丸"},
            复活 = {"佛光舍利子","九转回魂丹"},
   ...
   local 通过=false
      ...
      for i=1,3 do
       if  self.参战单位[编号].命魂之玉奇袭道具[i].参数==名称 then
             通过=true
             table.remove(self.参战单位[编号].命魂之玉奇袭道具,i)
             break
       end
      end
        ...
        if not 通过 then
           self:添加提示(玩家id, 编号, "#Y/错误，你没有此奇袭或已经使用过")
           return
        end
        ...
        if 使用类型=="加血" then
              if self.参战单位[目标].法术状态.魔音摄魂 or self.参战单位[目标].气血<=0 then
                    ...
                    使用=true
                    if 名称=="五龙丹" then
                            添加流程.挨打方[1].取消状态=self:解除状态组(编号,目标,self.技能数据.异常,"五龙丹",#self.战斗流程,1)
                            self:解除所有经脉异常(目标)
                            if #添加流程.挨打方[1].取消状态==0 then
                                self:添加状态("催眠符",目标,编号,150)
                                self.参战单位[目标].法术状态.催眠符.回合 = 5
                                self:处理流程状态(添加流程.挨打方[1],"催眠符",目标)
                            end
                            if self.参战单位[目标].奇经八脉.背水 then
                                  self:添加状态("背水",目标,编号,150)
                                  self:处理流程状态(添加流程.挨打方[1],"背水",目标)
                            end
...
function 战斗处理类:超级特技指令计算(编号,单位)
        if 单位.超级特技指令.类型=="道具" then
           self:超级特技道具计算(编号)
        elseif 单位.超级特技指令.类型=="特技" or 单位.超级特技指令.类型=="法术"then


           self:处理超级特技法术指令(编号,单位)
...
function 战斗处理类:处理超级特技法术指令(编号,单位)
    单位.超级特技指令.执行 = true
    local 名称 = 单位.超级特技指令.参数
    local 目标 = 单位.超级特技指令.目标
   单位.超级特技指令.名称=单位.超级特技指令.参数
    ...
    名称 = 单位.超级特技指令.参数
      ...
      if 单位.超级特技指令.类型=="特技" then
               local 通过=false

                  for i=1,3 do
                  if  self.参战单位[编号].命魂之玉奇袭特技[i] and self.参战单位[编号].命魂之玉奇袭特技[i].参数==单位.超级特技指令.名称 then
                  通过=true
                  table.remove(self.参战单位[编号].命魂之玉奇袭特技,i)
                  break
                  end
                  end
                  ...
                  if not 通过 then
                     self:添加提示(单位.玩家id, 单位.编号, "#Y/错误，你没有此奇袭或已经使用过")
                 return
                  end
    ...
    if (名称 == "狮搏" or 名称 == "鹰击" or 名称 == "连环击" or 名称 == "象形") and not 单位.法术状态.变身 then
        单位.超级特技指令.参数 = "变身"
        self:法术攻击处理(编号,单位.超级特技指令)
        self:添加提示(单位.玩家id, 单位.编号, "#Y/没有变身效果，本回合自动变身")
        return
    end
...
function 战斗处理类:回合结束处理(单位,单独发送)
          ...
          if 单位.气血 <= 0 and 单位.召唤兽 and 单位.奇经八脉.救主 and  self.参战单位[单位.召唤兽] and self.参战单位[单位.召唤兽].气血 > 0 and
              self:取行动状态(单位.召唤兽) then
              if 单位.门派 == "狮驼岭" and 取随机数() <= 50 and
              ...
              if 法宝.名称 == 名称 then
                  ...
                  if 数额 then
                      if (名称 == "金甲仙衣" and math.floor(境界*3) < 取随机数()) or
                         ...
                         (名称 == "嗜血幡" and math.floor(境界*3) < 取随机数()) then
                          return false
                      end
                  end
...
function 战斗处理类:取召唤兽可用法术(编号)
      local 可用法术 = {水攻 = true,烈火 = true,雷击 = true,落岩 = true,月光 = true,奔雷咒 = true,
                        ...
                        超级水漫金山 = true,超级三昧真火 = true,超级地狱烈火 = true,超级泰山压顶 = true,锤锤之光 = true}
